...
void Modbus::loop() {
  ...
  uint16_t timeout = this->frame_delay_ms_;
  if (this->rx_buffer_delay_override_ms_ != 0) {
    ...
    timeout = std::max(timeout, this->long_rx_buffer_delay_ms_);
  }
  ...
  if (!this->rx_buffer_.empty() && millis() - this->last_modbus_byte_ > timeout) {
    ...
    if (!this->rx_buffer_.empty()) {
      this->partial_response_timeout_count_++;
      {
...
int Modbus::check_frame_() const {
  ...
  if (len < 5)
    return 0;  // Smallest possible frame: addr(1)+FC(1)+exception(1)+CRC(2)
...
void Modbus::try_extract_frame_() {
  ...
  while (this->rx_buffer_.size() >= MIN_FRAME_SIZE) {
    ...
    ESP_LOGD(TAG, "Resync: dropping leading byte 0x%02X (%zu bytes remain)", this->rx_buffer_[0],
      ...
      if (this->role == ModbusRole::SERVER) {
        ...
        if ((function_code & FUNCTION_CODE_EXCEPTION_MASK) == FUNCTION_CODE_EXCEPTION_MASK) {
          ...
          if (this->waiting_for_response_ == address) {
            ...
            this->late_ignored_response_count_++;
...
void Modbus::send_next_frame_() {
  ...
  if (this->post_timeout_quarantine_) {
    if (this->available() || !this->rx_buffer_.empty() ||
        millis() - this->post_timeout_ts_ < this->frame_delay_ms_ + this->send_wait_time_ / 4) {
      return;
    }
...
void Modbus::dump_config() {
  ESP_LOGCONFIG(TAG,
                ...
                this->flow_control_pin_post_send_delay_ms_, this->rx_buffer_delay_override_ms_,
