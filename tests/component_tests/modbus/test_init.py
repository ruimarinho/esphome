...
def test_modbus_accepts_flow_control_delays(
    ...
    assert config[CONF_FLOW_CONTROL_PIN_PRE_SEND_DELAY] == cv.positive_time_period_milliseconds(
        "5ms"
    )
    assert config[CONF_FLOW_CONTROL_PIN_POST_SEND_DELAY] == cv.positive_time_period_milliseconds(
        "7ms"
    )
...
def test_modbus_accepts_rx_buffer_delay(
    ...
    assert config[CONF_RX_BUFFER_DELAY] == cv.positive_time_period_milliseconds(
        "25ms"
    )
