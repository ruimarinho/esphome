...
TEST(ModbusHelpersTest, PayloadToNumberRejectsOffsetAtEndOfBuffer) {
  ...
  EXPECT_EQ(modbus::helpers::payload_to_number(data, modbus::helpers::SensorValueType::U_WORD, 2, 0xFFFFFFFF), 0);
}

TEST(ModbusHelpersTest, PayloadToNumberRejectsTruncatedMultiRegisterValue) {
  const std::vector<uint8_t> data{0x12, 0x34, 0x56};
  EXPECT_EQ(modbus::helpers::payload_to_number(data, modbus::helpers::SensorValueType::U_DWORD, 0, 0xFFFFFFFF), 0);
}

TEST(ModbusHelpersTest, PayloadToNumberStillDecodesValidValues) {
  const std::vector<uint8_t> data{0x12, 0x34};
  EXPECT_EQ(modbus::helpers::payload_to_number(data, modbus::helpers::SensorValueType::U_WORD, 0, 0xFFFFFFFF),
            0x1234);
}
...
TEST(ModbusControllerTest, RejectsInvalidReadPayloadLength) {
  ...
  auto command =
      ModbusCommandItem::create_read_command(&controller, modbus::ModbusRegisterType::HOLDING, 0x0010, 1);
